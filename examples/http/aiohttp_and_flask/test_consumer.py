...
[`consumer.py`][examples.http.aiohttp_and_flask.consumer]) against a mock
...
from examples.http.aiohttp_and_flask.consumer import UserClient
...
@pytest.mark.asyncio
async def test_get_user(pact: Pact) -> None:
    """
    ...
    with pact.serve() as srv:
        async with UserClient(str(srv.url)) as client:
            user = await client.get_user(123)
            assert user.name == "Alice"


@pytest.mark.asyncio
async def test_get_unknown_user(pact: Pact) -> None:
    """
...
@pytest.mark.asyncio
async def test_create_user(pact: Pact) -> None:
    """
    ...
    with pact.serve() as srv:
        async with UserClient(str(srv.url)) as client:
            user = await client.create_user(name="Bob")
            assert user.id == 1000


@pytest.mark.asyncio
async def test_delete_user(pact: Pact) -> None:
    """
