...
from pact.error import RequestMismatch, RequestNotFound
from pact.pact import MismatchesError
...
async def test_basic_request_method(pact: Pact, method: str) -> None:
    (
        pact
        .upon_receiving(f"a basic {method} request")
        .with_request(method, "/")
...
async def test_basic_response_status(pact: Pact, status: int) -> None:
    (
        pact
        .upon_receiving(f"a basic request producing status {status}")
        .with_request("GET", "/")
...
async def test_with_header_request(
    ...
    (
        pact
        .upon_receiving("a basic request with a header")
        .with_request("GET", "/")
...
async def test_with_header_dict(pact: Pact) -> None:
    (
        pact
        .upon_receiving("a basic request with a headers from a dict")
        .with_request("GET", "/")
...
async def test_with_query_parameter_request(
    ...
    (
        pact
        .upon_receiving("a basic request with a query parameter")
        .with_request("GET", "/")
...
async def test_with_query_parameter_dict(pact: Pact) -> None:
    (
        pact
        .upon_receiving("a basic request with a query parameter from a dict")
        .with_request("GET", "/")
...
async def test_with_body_request(pact: Pact, method: str) -> None:
    (
        pact
        .upon_receiving(f"a basic {method} request with a body")
        .with_request(method, "/")
...
async def test_given(pact: Pact) -> None:
    (
        pact
        .upon_receiving("a basic request given state 1")
        .given("state 1")
    ...
    (
        pact
        .upon_receiving("a basic request given a user exists (1)")
    ...
    (
        pact
        .upon_receiving("a basic request given a user exists (2)")
...
async def test_binary_file_request(pact: Pact) -> None:
    ...
    (
        pact
        .upon_receiving("a basic request with a binary file")
        .with_request("POST", "/")
...
async def test_binary_file_response(pact: Pact) -> None:
    ...
    (
        pact
        .upon_receiving("a basic request with a binary file response")
        .with_request("GET", "/")
        .will_respond_with(200)
...
@pytest.mark.asyncio
async def test_multipart_file_request(pact: Pact, temp_assets: Path) -> None:
    fpy = temp_assets / "test.py"
    fpng = temp_assets / "test.png"
    (
        pact
        .upon_receiving("a basic request with a multipart file")
        .with_request("POST", "/")
        .with_multipart_file(
            fpy.name,
            fpy,
...
async def test_name(pact: Pact) -> None:
    (
        pact
        .upon_receiving("a basic request with a test name")
...
async def test_with_plugin(pact: Pact) -> None:
    (
        pact
        .upon_receiving("a basic request with a plugin")
