...
from pact.v2.verify_wrapper import VerifyWrapper
...
class VerifierPactsTestCase(TestCase):
    ...
    @patch('pact.v2.verifier.path_exists', return_value=False)
    ...
    @patch("pact.v2.verify_wrapper.VerifyWrapper.call_verify", return_value=(0, None))
    @patch('pact.v2.verifier.expand_directories', return_value=['./pacts/pact1', './pacts/pact2'])
    @patch('pact.v2.verifier.path_exists', return_value=True)
    ...
    def test_passes_enable_pending_flag_value(self, mock_wrapper):
        for value in (True, False):
            with self.subTest(value=value):
                with patch('pact.v2.verifier.path_exists'):
