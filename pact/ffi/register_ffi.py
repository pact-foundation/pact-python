...
import os
import sys
from pact.constants import FFI_LIB_PATH
...
class RegisterFfi(object):
    ...
    def load_ffi_library(self, ffi):
        ...
        if ("darwin" in target_platform or "macos" in target_platform) and ("aarch64" in platform.machine() or "arm64" in platform.machine()):
            ...
            libname = os.path.join(FFI_LIB_PATH, "libpact_ffi-linux-x86_64.so")
        elif 'windows' in target_platform:
            libname = os.path.join(FFI_LIB_PATH, "pact_ffi-windows-x86_64.dll")
        else:
