...
class PactBuildHook(BuildHookInterface[Any]):
    ...
    def _install(self, version: str) -> dict[str, str]:
        ...
        if pact_lib_dir := os.getenv("PACT_LIB_DIR"):
            # Copy the library to make it available by other processes
            dir_path = Path(pact_lib_dir)
            dir_path.mkdir(parents=True, exist_ok=True)
            self.app.display_debug(f"Copying {lib.name} into current {dir_path}")
            shutil.copy(lib, dir_path / lib.name)

        return {str(extension): f"pact_ffi/{extension_name}.abi3.{suffix}"}
    ...
    def _extract_lib(self, artifact: Path) -> Path:
        ...
        """
        target = self.tmpdir / (artifact.name.split("-")[0] + artifact.suffixes[0])
        with (
